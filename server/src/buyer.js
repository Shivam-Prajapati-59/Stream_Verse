...
const fetchWithPayment = wrapFetchWithPayment(fetch, client, { facilitatorUrl: FACILITATOR_URL });
...
fetchWithPayment(url, { //url should be something like https://api.example.com/paid-endpoint
    ...
    .then(async response => {
        ...
        const res = JSON.parse(JSON.stringify(paymentResponse));
        console.log(res);
    })
    .catch(async error => {
        ...
        console.error('Error config:', error.status);
        console.error('Error details:', error.headers);
        if (error.response) {
